...
export function addDamageNumber(x, y, damage, isCrit = false, isHeal = false) {
    ...
    console.log(`💫 Added damage number: ${damage} (crit: ${isCrit}, heal: ${isHeal})`);
}
...
export function triggerScreenShake(intensity = 5, duration = 200) {
    ...
    console.log(`💥 Screen shake triggered: intensity=${intensity}, duration=${duration}ms`);
}
...
export function clearAllEffects() {
    ...
    console.log('🧹 All combat effects cleared');
}
...
export function triggerCritEffect(x, y, damage) {
    ...
    console.log('✨ Critical hit effect combo triggered!');
}
...
export function triggerHealEffect(x, y, healAmount) {
    // Heal number (green, floating up)
    addDamageNumber(x, y, healAmount, false, true);
    
    console.log(`❤️‍🩹 Heal effect: ${healAmount}`);
}
...
export function getEffectStats() {
    return {
        damageNumbers: activeEffects.damageNumbers.length,
        hitFlashes: activeEffects.hitFlashes.length,
        screenShakeActive: activeEffects.screenShake.active
    };
}
